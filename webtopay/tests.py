...
from forms import WebToPaymentForm
...
class TestRequest(TestCase):
    def testPassword(self):
        query['ss1'] = query.pop('wp__ss1')
        query['ss2'] = query.pop('wp__ss2')
        form = WebToPaymentForm(password=SIGN_PASSWORD, **query)
        form.sign_with_password()
        self.assertEqual(query['ss1'], form.data['sign'])
